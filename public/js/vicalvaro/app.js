vm = new Vue({
	...
	methods: {
		edit: function(id)
		{
			...
			this.$http.get('/vicalvaro/edit/'+ id, function(response){
		...
		borrar: function(id)
		{
            ...
            function(){ 
			vm.report_id = id;
			vm.$http.get('/vicalvaro/delete/'+ id, function(response){
				location.reload();
			});
                swal("Deleted!", 
                    "Ha sido borrado.",
		...
		{
			this.$http.post('/getErrors' ,{id: id , entorno: 'vicalvaro'},  function(errors){
				//console.log(errors);
				vm.errors = errors;
			})		}
		,
		...
		getReports: function()
		{
			this.$http.get('/vicalvaro/reports' , function(reports){
				//console.log(reports);
				vm.reports = reports;
			})
		}
