vm = new Vue({
	...
	methods: {
		edit: function(id)
		{
			...
			this.$http.get('/srvmast/edit/'+ id, function(response){
		...
		borrar: function(id)
		{
            ...
            function(){ 
     		vm.report_id = id;
			vm.$http.get('/srvmast/delete/'+ id, function(response){
				location.reload();
			});
                swal("Deleted!", 
                    "Ha sido borrado.",
		...
		{
			this.$http.post('/getErrors' ,{id: id , entorno: 'srvmast'},  function(errors){
				//console.log(errors);
				vm.errors = errors;
			})		}
		,
		...
		getReports: function()
		{
			this.$http.get('/srvmast/reports' , function(reports){
				//console.log(reports);
				vm.reports = reports;
			})
		}
